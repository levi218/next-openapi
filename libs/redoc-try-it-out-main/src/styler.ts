...
export class Styler {
  ...
  private static authApplies(): void {
    StyleBuilder.add(SwaggerWrapper.cfg.shownAuthModalSelector)
      .hidden()
  ...
  private static tryBtnApplies(): void {
    StyleBuilder.add(TryBtn.cfg.selector)
      .resetBorder()
  ...
  private static inputApplies(): void {
    StyleBuilder.add(Styler.cfg.inputTargetSelector)
  ...
  private static modalApplies(): void {
    StyleBuilder.add(Styler.cfg.modalHeaderTargetSelector)
  ...
  private static tryItBoxApplies(): void {
    StyleBuilder.add(Styler.cfg.operationSectionTargetSelector)
      .hidden()
      .noHeight()
    ...
    StyleBuilder.add(Styler.cfg.openedOperationContainerSelector).style({
      display: 'block',
    });

    StyleBuilder.add(Styler.cfg.operationContainerTargetSelector)
  ...
  private static hideDisturbingElements(): void {
    StyleBuilder.add(Styler.cfg.operationTryOutTargetSelector)
      .noDisplay()
  ...
  public static createStyleElements(): void {
    setTimeout(function () {
      ...
      marker.insertBefore(
        `${Styler.cfg.parameterNameFieldTargetSelector}:first-child`
      );
    }, 100);
